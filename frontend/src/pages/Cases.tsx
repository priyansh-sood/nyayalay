...
const inputCls =
  "w-full px-3 py-2 rounded-lg border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring transition";

// Field defined OUTSIDE the parent component so React never remounts it
const Field = ({
  label,
  children,
}: {
...
// Modal extracted into its own component so its renders are isolated
...
const CaseModal = memo(function CaseModal({
  editCase,
  initialData,
  onClose,
  onSaved,
  ...
  const set = (field: keyof CaseFormData) =>
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
      const value =
        field === "priority_score" || field === "estimated_duration_minutes"
    ...
    try {
      const payload = {
        ...
        next_date: formData.next_date
          ? new Date(formData.next_date).toISOString()
          : null,
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Field label={t("cases.caseNumber")}>
              <input
                ...
                placeholder="Sessions Case No. 45/2024"
                className={inputCls}
              />
            </Field>
          ...
          <Field label={t("cases.courtName")}>
            <input
              ...
              placeholder="Additional Sessions Court, Saket, New Delhi"
              className={inputCls}
            />
          </Field>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Field label={t("cases.petitioner")}>
              <input
                ...
                placeholder="State / Complainant name"
                className={inputCls}
              />
            </Field>
            <Field label={t("cases.respondent")}>
              <input
                ...
                placeholder="Accused / Defendant name"
                className={inputCls}
              />
            </Field>
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <Field label={t("cases.ipcSections")}>
              <input
                ...
                onChange={set("ipc_sections")}
                placeholder="302, 34, 120B"
                className={inputCls}
              />
            </Field>
...
export default function Cases() {
  ...
  const [modalInitialData, setModalInitialData] = useState<CaseFormData>(EMPTY_FORM);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="relative">
          ...
          >
            {STATUSES.map((s) => (
              <option key={s} value={s}>
                {s ? t(`status.${s}`) : t("cases.filterStatus")}
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="relative">
          ...
          >
            {PRIORITIES.map((p) => (
              <option key={p} value={p}>
                {p ? t(`priority.${p}`) : t("cases.filterPriority")}
              </option>
            ))}
          </select>
        </div>
      ...
      {/* Modal — isolated component so typing doesn't trigger parent re-render */}
      {showModal && (
        <CaseModal
          editCase={editCase}
