...
def index_document(doc_id: int, case_id: int, text: str, metadata: dict) -> bool:
    """Chunk text and upsert embeddings into Pinecone."""
...
def query_documents(
    ...
    """Query Pinecone and return top-k relevant chunks with metadata."""
    index = get_pinecone_index()
    if index is None:
        return []
...
def generate_rag_answer(query: str, contexts: List[Dict[str, Any]], case_id: Optional[int] = None) -> str:
    """Use retrieved context + GPT to answer legal queries."""
    if not OPENAI_API_KEY:
        return _fallback_answer(query, contexts)

    if not contexts:
        return (
            "No relevant documents found in the system. "
            "Please upload case documents to enable legal research queries."
        )
    ...
    system_prompt = (
        ...
        "CrPC, CPC, and landmark judgments. Answer legal queries based on the provided document "
        "context. Cite the source document filename when referencing specific passages. "
        "Be precise, cite relevant IPC sections if applicable, and format your answer clearly."
    )

    user_prompt = (
        f"Based on the following legal documents, answer this query:\n\n"
        f"Query: {query}\n\n"
        f"Document Context:\n{context_text[:6000]}"
    )
...
def _fallback_answer(query: str, contexts: List[Dict[str, Any]]) -> str:
    ...
    return (
        ...
        f"{top['text']}\n\n"
        f"[AI summary unavailable – configure OPENAI_API_KEY for full RAG answers]"
    )
